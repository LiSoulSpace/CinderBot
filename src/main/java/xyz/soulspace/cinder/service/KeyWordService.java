...
import xyz.soulspace.cinder.constant.ConstantFreeTime;
...
public class KeyWordService {
    ...
    @Autowired
    private CinderBotService cinderBotService;
    ...
    private boolean groupRepeater(GroupMessageEvent event) {
        ...
        for (SingleMessage singleMessage : event.getMessage()) {
            ...
            if (singleMessage.getClass() == PlainText.class) {
                //普通文本 直接拼接
                sbMsg.append(((PlainText) singleMessage).getContent());
            } else if (singleMessage.getClass() == OnlineGroupImageImpl.class) {
                //图片
                sbMsg.append(((OnlineGroupImageImpl) singleMessage).getImageId());
            } else if (singleMessage.getClass() == Face.class) {
                //qq表情
    ...
    private boolean groupKeyWordImage(GroupMessageEvent event) throws IOException {
        ...
        //上传图片
        Image miraiImage = cinderBotService.uploadMiraiImage(imgLocalPath);
