...
@Tag(name = "资源控制器(ResourceController)")
public class ResourceController {
    ...
    public ResponseEntity<?> create(@RequestBody Resource umsResource) {
        boolean save = resourceService.save(umsResource);
    ...
    public ResponseEntity<?> update(@PathVariable Long id,
        ...
        boolean update = resourceService.updateById(umsResource);
        dynamicSecurityMetadataSource.clearDataSource();
        if (update) {
            return new ResponseEntity<>(true, HttpStatus.OK);
        } else {
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        boolean remove = resourceService.removeById(id);
        dynamicSecurityMetadataSource.clearDataSource();
        if (remove) {
            return new ResponseEntity<>(true, HttpStatus.OK);
        } else {
